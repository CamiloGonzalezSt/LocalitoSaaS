  ...
  return <div className="stack">
    ...
    <section className="panel">
      ...
      {!cashSession ? <div className="form-grid"><input inputMode="numeric" value={openingAmount} onChange={(event) => setOpeningAmount(event.target.value)} placeholder="Monto inicial"/><button className="primary-action" disabled={busy} onClick={() => run(() => api.openCashSession(Number(openingAmount)), "Caja abierta.")}>Abrir caja</button></div> : <>
        ...
        <CashReconciliation sessionId={cashSession.id} revision={cashMovements.length} onClosed={async () => { await Promise.all([load(), onRefresh()]); setMessage("Caja cerrada y conciliación guardada."); }}/>
      </>}
    </section>
    ...
    {canManage && <ReplenishmentPanel products={products} sales={sales} purchases={purchases} preferences={preferences} onProposal={lines => { setProposalLines(lines); document.querySelector(".purchase-proposal-review")?.scrollIntoView({ block: "center" }); }}/>}
    {canManage && <AuditBrowser events={audit}/>}
    {canManage && <section className="panel">
      ...
      <div className="list">
        ...
        {!expiringSoon.length && <p className="empty-state">Sin vencimientos a 30 días.</p>}
      </div>
    </section>}
