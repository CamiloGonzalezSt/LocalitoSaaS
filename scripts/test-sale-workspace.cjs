...
(async () => {
  ...
  try {
    ...
    assert.equal(await page.getByRole('button', { name: 'Confirmar cobro', exact: true }).isDisabled(), true);
    await page.getByRole('checkbox').check();
    assert.equal(await page.getByRole('button', { name: 'Confirmar cobro', exact: true }).isEnabled(), true);
    await page.getByRole('button', { name: 'Volver al ticket', exact: true }).click();
    await page.getByLabel('Descuento', { exact: true }).fill('200');
    await page.getByRole('button', { name: /^Cobrar \$/ }).click();
    assert.equal(await page.getByRole('checkbox').isChecked(), false);
    await page.getByRole('button', { name: 'Efectivo', exact: true }).click();
    await page.getByRole('button', { name: 'Monto exacto', exact: true }).click();
    ...
    assert.equal((await stored()).active.items.length, 0);
    assert.equal(await page.evaluate(async () => (await import('/src/lib/offline.ts')).queueEntries().length), 1);
    await page.unroute('**/sales');
    await page.reload();
    await page.locator('.sale-products-panel').waitFor();
    assert.equal((await stored()).active.items.length, 0);
    assert.equal(await page.evaluate(async () => (await import('/src/lib/offline.ts')).queueEntries().length), 0);
